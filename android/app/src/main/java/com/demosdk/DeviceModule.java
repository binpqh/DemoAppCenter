...
public class DeviceModule extends ReactContextBaseJavaModule {
    ...
    public DeviceModule(ReactApplicationContext reactContext) {
        ...
        listSimDispenserMain = Arrays.asList(new SimdispenserMain(), new SimdispenserMain(), new SimdispenserMain(),
                new SimdispenserMain());
    }
    ...
    public String getStatusUPS() {
        ...
        assert upsData != null;
        Log.d("BinXiudeptrai",
                "totalInfo :" + upsData.totalInfo + " batteryVoltage :" + upsData.batteryVoltage + " inputVoltage :"
                        + upsData.inputVoltage + " outputVoltage: " + upsData.outputVoltage
                        + " consumedLoad: " + upsData.consumedLoad + " " + upsData.frequencyOutput);
        return "totalInfo :" + upsData.totalInfo + " batteryVoltage :" + upsData.batteryVoltage + " inputVoltage :"
                + upsData.inputVoltage + " outputVoltage: " + upsData.outputVoltage
                + " consumedLoad: " + upsData.consumedLoad + " " + upsData.frequencyOutput;
    }
    ...
    public void install(String path) {
        ...
        Uri apkUri = FileProvider.getUriForFile(context,
                context.getApplicationContext().getPackageName() + ".fileprovider", new File(path));
    ...
    public String initSimDispenser() {
        // Log.d("DeviceModule", "initSimDispenser: " + timeToRecyleCard + ", " +
        // comName + ", " + baurate + ", " + idSimDispenser);
    ...
    public String getStatus(int idSimDispenser) {
        ...
        HashMap<String, Object> getStatusResult = listSimDispenserMain.get(idSimDispenser).m_control
                .controlCheckSensorStatus(listSimDispenserMain.get(idSimDispenser));
    ...
    public boolean moveCardToBoxCheck(int idSimDispenser) {
        ...
        return listSimDispenserMain.get(idSimDispenser).m_control
                .controlICCardPosiion(listSimDispenserMain.get(idSimDispenser));
    }
    ...
    public void readSerialSim(int idSimDispenser, Callback callback) {
        ...
        HashMap<String, Object> readSeriSimResult = listSimDispenserMain.get(idSimDispenser).m_control
                .controlReadICCIDSim(listSimDispenserMain.get(idSimDispenser));
    ...
    public void retainCard(int idSimDispenser, Callback callback) {
        ...
        boolean retainCard = listSimDispenserMain.get(idSimDispenser).m_control
                .controlRetainCard(listSimDispenserMain.get(idSimDispenser));
    ...
    public boolean moveToFront(int idSimDispenser) {
        ...
        return listSimDispenserMain.get(idSimDispenser).m_control
                .controlClipPosiion(listSimDispenserMain.get(idSimDispenser));
    }
    ...
    @ReactMethod
    public boolean printReceipt(String kioskId, int deviceId, int vendorId, int productId, String receiptNumber,
            String language) {
        if (language == null)
            language = "en";
        ...
        if (!status.isIsOutOfPaper()) {
            SDKPrints.print(vendorId, productId, deviceId, null, commodityList, total, kioskId, receiptNumber, context,
                    mPos, mUsb, language);
            return true;
        }
        return false;
    ...
    public String getTemperature() {
        ...
        while (maxTry < 5 && temphuData == null) {
            ...
            for (DeviceInfo each : devices) {
                ...
                Log.d("Device Information", "Name : " + each.device.getDeviceName() + " Id : "
                        + each.device.getVendorId() + " ProductId : " + each.device.getProductId());
                if (temphuDeviceId == -1 && each.device.getVendorId() == temphuVendorId
                        && each.device.getProductId() == temphuProductId) {
                ...
                if (temphuDeviceId != -1 && each.device.getVendorId() == temphuVendorId
                        && each.device.getProductId() == temphuProductId
                        && Utils.compareTwoDeviceId(each.device.getDeviceId(), temphuDeviceId)) {
                    if (connect) {
                        temphuData = tempHuSDK.getTempHumiData();
                    }
                }
                if (temphuData != null) {
                    result.append("Device : ").append(each.device.getDeviceId()).append("\nTemperature")
                            .append(temphuData.temperature).append("\nHumidity").append(temphuData.humidity)
                            .append("\nDewdrop").append(temphuData.dewdrop);
                    Log.d("Temperature",
                            "Device : " + each.device.getDeviceId() + "\nTemperature" + temphuData.temperature
